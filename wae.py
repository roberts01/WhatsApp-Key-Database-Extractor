...
if(len(output) == 1):
    print('Could not find any connected device. Is USB Debugging on?')
    ADBSerialId = ''
...
if(len(output) == 2):
    if(output[1].split()[1] == 'offline'):
        print('Device is offline, try turning off USB debugging and turn on again.')
        Exit()
    if(output[1].split()[1] == 'unauthorized'):
        print('Device unauthorized. Please check the confirmation dialog on your device.')
        Exit()
...
if deviceToConnect is None:
    for device in output[1:]:
        ...
        print(str(i) + '. ' + device.split()[0] + '  ' + device.split()[1] + '  ' + sp.getoutput(name).strip())
        i += 1

while deviceToConnect is None:
    deviceIndex = int(input('Enter device number (for ex : 2) : '))
...
def CheckJAVA() : 
    ...
    if (isJAVAInstalled): 
        ...
        print('It looks like you don\'t have JAVA installed on your system. Please install it first.')
        Exit()
...
if __name__ == "__main__":
    ...
    if (SDKVersion <= 13) :
        print('Unsupported device. This method only works on Android v4.0 or higer.')
        print('Cleaning up temporary direcory.')
    ...
    if not (WhatsAppapkPath): 
        print('Looks like WhatsApp is not installed on device.')
        Exit()
    ...
    print('WhatsApp Version' + versionName + ' installed on device') 
    ACReturnCode = 1
    ...
    if ACReturnCode==1:
        ...
        print('Backing up WhatsApp ' + versionName + ' apk, the one installed on device to ' + tmp + 'WhatsAppbackup.apk')
        os.system(adb + ' pull ' + WhatsAppapkPath + ' ' + tmp + 'WhatsAppbackup.apk')
        print('Apk backup complete.')

        #UninstallWhatsApp
        if(SDKVersion >= 23) :
            try : 
                print('Uninstalling WhatsApp, skipping data.')
                os.system(adb + ' shell pm uninstall -k com.whatsapp')
                print('Uninstalled.')
            except Exception as e : 
                print('Could not uninstall WhatsApp.')
                print(e)
                Exit()

        #InstallLegacyWhatsapp
        print("installing Legacy WhatsApp v2.11.431...")
        if(SDKVersion >= 17) :
        ...
        print('Backing up WhatsApp data as ' + tmp + 'whatsapp.ab...')
        try:
            if(SDKVersion >= 23):
            ...
            print(e)
        print('Done backing up data.')

        #Reinstall WhatsApp
        print('Reinstallting original WhatsApp...')
        try : 
            os.system(adb + ' install -r -d ' + tmp + 'WhatsAppbackup.apk')
        except Exception as e : 
            print(e)
            print('Could not install WhatsApp, install by running \'restore_whatsapp.py\' or manually installing from Play Store.\nHowever if it crashes then you have to clear storage/clear data from settings => app settings => WhatsApp.')    

        #print('Our work with device has finished.')
